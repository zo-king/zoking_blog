...
function AuthenticatedAdmin({
  ...
  const {
    ...
    setError,
    refresh,
  } = useAdminData(token, setCurrentUser, settingsForm, expireSession, section, editorID, section === "settings");
  ...
  const achievementAdmin = useAchievementAdmin({
    ...
    read:
      ...
      can("achievement:read") &&
      (!editorID || (editorID === "new" ? can("achievement:create") : can("achievement:update"))),
  ...
  const {
    ...
    moderateComment,
    deleteComment,
  } = useContentAdminCommands({
    token,
    canManageTaxonomy: can("taxonomy:manage"),
    canUploadMedia: can("media:upload"),
    canDeleteMedia: can("media:delete"),
    canInsertMediaMarkdown: can("post:update"),
    canModerateComments: can("comment:moderate"),
    refresh,
    setError,
    setMedia,
    setComments,
  ...
  const {
    ...
    retryPublishJob,
    cancelPublishJob,
  } = usePublishingCommands({ token, refresh, setError });
  const {
    busy,
    deletingPostID,
    pageBusy,
    ...
    openPreviewURL,
    resetEditors,
  } = useEditorialCommands({
    token,
    canUpdateSettings: can("setting:update"),
    refresh,
    setError,
    postForm: form,
    pageForm,
  ...
  const navItems = useMemo(() => {
    ...
    return definitions.filter(
      (item) => !item.permission || Boolean(currentUser?.permissions.includes(item.permission)),
    );
  }, [currentUser]);
  ...
  const activePost =
    section === "posts" && editorID && editorID !== "new" ? posts.find((post) => post.id === editorID) : undefined;
  const seriesOptions = useMemo(() => {
    const currentSeries = activePost?.series;
    const availableSeries =
      currentSeries && !series.some((item) => item.id === currentSeries.id) ? [...series, currentSeries] : series;
  ...
  const activePage =
    section === "pages" && editorID && editorID !== "new" ? pages.find((page) => page.id === editorID) : undefined;
  ...
  return (
    ...
    >
      ...
      {identityReady && !routeAllowed && (
        <Alert type="warning" showIcon title="无权访问" content="当前账号没有访问此模块所需的权限。" />
      )}

      {identityReady && section === "dashboard" && <DashboardPage health={health} ready={ready} />}

      {routeAllowed && section === "posts" && (
        <PostsPage
          posts={posts}
          ...
          onDelete={deletePost}
          onSave={(values) =>
            void saveDraft(values).then((saved) => {
              if (saved && editorID === "new") navigate(`/posts/${saved.id}/edit`, { replace: true });
            })
          }
          onPreview={() =>
            void previewPostDraft().then((saved) => {
              if (saved && editorID === "new") navigate(`/posts/${saved.id}/edit`, { replace: true });
            })
          }
          onPublish={() =>
            void publish().then((saved) => {
              if (saved && editorID === "new") navigate(`/posts/${saved.id}/edit`, { replace: true });
            })
          }
      ...
      {routeAllowed && section === "pages" && (
        <PagesPage
          ...
          onSave={(values) =>
            void savePage(values).then((saved) => {
              if (saved && editorID === "new") navigate(`/pages/${saved.id}/edit`, { replace: true });
            })
          }
          onPreview={() =>
            void previewPageDraft().then((saved) => {
              if (saved && editorID === "new") navigate(`/pages/${saved.id}/edit`, { replace: true });
            })
          }
          onPublish={() =>
            void publishPage().then((saved) => {
              if (saved && editorID === "new") navigate(`/pages/${saved.id}/edit`, { replace: true });
            })
          }
      ...
      {routeAllowed && section === "achievements" && (
        <AchievementsPage
          ...
          onSave={(values) =>
            void achievementAdmin.save(values, editorID === "new" ? undefined : editorID).then((saved) => {
      ...
      {sectionAllowed && section === "taxonomy" && (
        <TaxonomyPage
      ...
      {sectionAllowed && section === "comments" && (
        <CommentsPage
          comments={comments}
      ...
      {sectionAllowed && section === "settings" && (
        <SettingsPage
          form={settingsForm}
      ...
      {sectionAllowed && section === "publishing" && (
        <PublishingPage
          jobs={publishJobs}
      ...
      <Modal
        title={`重置密码：${passwordResetUser?.display_name || passwordResetUser?.username || ""}`}
        visible={Boolean(passwordResetUser)}
        onCancel={closePasswordReset}
        onOk={() => passwordForm.submit()}
        confirmLoading={userBusy}
        okText="确认重置"
        cancelText="取消"
        unmountOnExit
      >
        <Form form={passwordForm} layout="vertical" onSubmit={(values) => void resetUserPassword(values)}>
          <Form.Item
            field="password"
            label="新密码"
            extra="至少 10 个字符；重置后会撤销该用户现有刷新令牌。"
            rules={[{ required: true, minLength: 10, maxLength: 72 }]}
          >
      ...
      <Modal
        ...
        confirmLoading={roleBusy}
        okText="保存"
        cancelText="取消"
        unmountOnExit
      >
        <Form form={roleEditForm} layout="vertical" onSubmit={(values) => void updateRole(values)}>
          <Form.Item field="name" label="角色名称" rules={[{ required: true, maxLength: 120 }]}>
            <Input />
          </Form.Item>
          <Form.Item field="description" label="说明" rules={[{ maxLength: 500 }]}>
      ...
      <ContentQualityPanel
        ...
        targetLabel={String(
          qualityTarget === "post"
            ? form.getFieldValue("title") || "当前文章"
            : pageForm.getFieldValue("title") || "当前页面",
        )}
