...
export function useAchievementAdmin(options: UseAchievementAdminOptions) {
    ...
    try {
      if (editorID && editorID !== "new") {
        ...
        const result = await apiFetch<ApiEnvelope<Achievement[]>>(
          `/api/v1/admin/achievements${queryString ? `?${queryString}` : ""}`,
          {},
          token,
        );
        setAchievements(result.data);
  ...
  const save = useCallback(
    async (values: AchievementFormValues, id?: string) => {
  ...
  const updateStatus = useCallback(
    async (id: string, status: string) => {
      if (!token || !canPublish) {
        Message.warning("当前账号无权修改成果状态");
        return false;
      }
      setBusy(true);
      try {
        await apiFetch<ApiEnvelope<Achievement>>(
  ...
  const searchMedia = useCallback(
    async (q: string, page: number, pageSize = 12): Promise<MediaSearchResult> => {
