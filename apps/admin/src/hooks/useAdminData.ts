...
function paginationFrom<T>(result: ApiEnvelope<T[]>, fallbackPageSize = 20): PaginationMeta {
  ...
  return (
    result.pagination || {
      ...
      total,
      total_pages: total === 0 ? 0 : Math.ceil(total / fallbackPageSize),
    }
  );
}
...
export function useAdminData(
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const currentUserResult = token
        ? await apiFetch<ApiEnvelope<CurrentUser>>("/api/v1/admin/auth/me", {}, token)
        : ({ data: null, request_id: "" } as unknown as ApiEnvelope<CurrentUser>);
      const granted = (permission: string) => Boolean(currentUserResult.data?.permissions.includes(permission));
      const [postPath, categoryPath, tagPath] =
        token && granted("post:read")
          ? ["/api/v1/admin/posts", "/api/v1/admin/categories", "/api/v1/admin/tags"]
          : ["/api/v1/public/posts", "/api/v1/public/categories", "/api/v1/public/tags"];
      const pagePath = token && granted("page:read") ? "/api/v1/admin/pages" : "/api/v1/public/pages";
      const settingsPath =
        token && granted("setting:read") ? "/api/v1/admin/settings" : "/api/v1/public/site/public-settings";
      ...
      if (!editorAllowed) {
        if (section === "posts") {
          ...
          setSeries([]);
          setMedia([]);
          setPostPagination(emptyPagination);
        }
        if (section === "pages") {
          setPages([]);
          setPagePagination(emptyPagination);
        }
        return;
      }
      ...
      if (section === "dashboard") {
        ...
        setPostPagination(paginationFrom(postResult, 1));
        setPagePagination(paginationFrom(pageResult, 1));
      } else if (section === "posts") {
        const mediaRequest =
          token && granted("media:read")
            ? apiFetch<ApiEnvelope<MediaAsset[]>>("/api/v1/admin/media?page=1&page_size=100", {}, token)
            : empty<MediaAsset>();
        ...
        const postRequest =
          editorID === "new"
            ? empty<Post>()
            : editorID
              ? apiFetch<ApiEnvelope<Post>>(`/api/v1/admin/posts/${editorID}`, {}, token).then((result) => ({
                  ...result,
                  data: [result.data],
        ...
        setPostPagination(paginationFrom(postResult));
      } else if (section === "pages") {
        const pageResult =
          editorID === "new"
            ? await empty<Page>()
            : editorID
              ? await apiFetch<ApiEnvelope<Page>>(`/api/v1/admin/pages/${editorID}`, {}, token).then((result) => ({
                  ...result,
                  data: [result.data],
                  pagination: { page: 1, page_size: 1, total: 1, total_pages: 1 },
                }))
        ...
        setPages(pageResult.data);
        setPagePagination(paginationFrom(pageResult));
        ...
        const [categoryResult, tagResult, seriesResult, mediaResult] = await Promise.all([
          apiFetch<ApiEnvelope<Taxonomy[]>>(
            adminTaxonomy ? "/api/v1/admin/categories" : "/api/v1/public/categories",
            {},
            token || undefined,
          ),
          apiFetch<ApiEnvelope<Taxonomy[]>>(
            adminTaxonomy ? "/api/v1/admin/tags" : "/api/v1/public/tags",
            {},
            token || undefined,
          ),
          apiFetch<ApiEnvelope<Series[]>>(
            adminTaxonomy ? "/api/v1/admin/series" : "/api/v1/public/series",
            {},
            token || undefined,
          ),
          mediaRequest,
        ...
        const result =
          token && granted("media:read")
            ? await apiFetch<ApiEnvelope<MediaAsset[]>>(`/api/v1/admin/media${search}`, {}, token)
            : await empty<MediaAsset>();
        setMedia(result.data);
        setMediaPagination(paginationFrom(result));
      } else if (section === "comments") {
        const result =
          token && granted("comment:read")
            ? await apiFetch<ApiEnvelope<Comment[]>>(`/api/v1/admin/comments${search}`, {}, token)
            : await empty<Comment>();
        setComments(result.data);
        setCommentPagination(paginationFrom(result));
      } else if (section === "publishing") {
        const [previewResult, jobResult, releaseResult] =
          token && granted("publish:read")
            ? await Promise.all([
        ...
        setPublishPreviews(previewResult.data);
        setPublishJobs(jobResult.data);
        setPublishReleases(releaseResult.data);
        setPreviewPagination(paginationFrom(previewResult));
        setJobPagination(paginationFrom(jobResult));
        setReleasePagination(paginationFrom(releaseResult));
      } else if (section === "users") {
        const [userResult, roleResult, permissionResult] = await Promise.all([
          token && granted("user:read")
            ? apiFetch<ApiEnvelope<AdminUser[]>>(`/api/v1/admin/users${search}`, {}, token)
            : empty<AdminUser>(),
          token && granted("role:read")
            ? apiFetch<ApiEnvelope<SystemRole[]>>("/api/v1/admin/roles", {}, token)
            : empty<SystemRole>(),
          token && granted("role:read")
            ? apiFetch<ApiEnvelope<string[]>>("/api/v1/admin/permissions", {}, token)
            : empty<string>(),
        ]);
        setAdminUsers(userResult.data);
        setSystemRoles(roleResult.data);
        setPermissions(permissionResult.data);
        setUserPagination(paginationFrom(userResult));
      } else if (section === "settings") {
        const settingsResult = await apiFetch<ApiEnvelope<SiteSettingsResponse>>(settingsPath, {}, token || undefined);
        setSiteSettings(settingsResult.data.settings);
        setSiteSettingsHash(settingsResult.data.hash);
        if (settingsFormConnected) settingsForm.setFieldsValue(settingsResult.data.settings);
      } else if (section === "audit") {
        const result =
          token && granted("audit:read")
            ? await apiFetch<ApiEnvelope<AuditLog[]>>(`/api/v1/admin/audit-logs${search}`, {}, token)
            : await empty<AuditLog>();
        setAuditLogs(result.data);
        setAuditPagination(paginationFrom(result));
      }
    } catch (err) {
      if (section === "dashboard") {
        setPosts([]);
