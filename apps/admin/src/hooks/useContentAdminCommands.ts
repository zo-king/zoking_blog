...
export function useContentAdminCommands({
  token,
  canManageTaxonomy,
  canUploadMedia,
  canDeleteMedia,
  canInsertMediaMarkdown,
  canModerateComments,
  refresh,
  setError,
  setMedia,
  ...
  canInsertMediaMarkdown: boolean;
  ...
  async function createTaxonomy(path: "categories" | "tags", values: CategoryFormValues | TagFormValues) {
    ...
    try {
      await apiFetch<ApiEnvelope<Taxonomy>>(
        `/api/v1/admin/${path}`,
  ...
  async function deleteTaxonomy(path: "categories" | "tags", id: string) {
    ...
    setTaxonomyBusy(true);
    try {
      await apiFetch(`/api/v1/admin/${path}/${id}`, { method: "DELETE" }, token);
      Message.success(path === "categories" ? "分类已删除" : "标签已删除");
      await refresh();
    } catch (err) {
      setError(String(err));
    } finally {
  ...
  function seriesError(error: unknown) {
    if (error instanceof ApiError && error.status === 409) {
      if (error.code === "SERIES_REFERENCED" || error.code === "SERIES_IN_USE")
        return "系列删除失败：该系列仍被文章引用，请先移除文章关联。";
  ...
  async function saveSeries(values: SeriesFormValues, id?: string): Promise<boolean> {
    ...
    try {
      ...
      const result = await apiFetch<ApiEnvelope<Series>>(
        id ? `/api/v1/admin/series/${id}` : "/api/v1/admin/series",
  ...
  async function uploadMediaFile(options: {
    ...
    if (!requirePermission(canUploadMedia)) {
      options.onError?.(new Error("当前账号无权上传媒体"));
      return;
    }
  ...
  async function deleteMedia(id: string) {
    if (!requirePermission(canDeleteMedia)) return;
    setMediaBusy(true);
    try {
      await apiFetch(`/api/v1/admin/media/${id}`, { method: "DELETE" }, token);
  ...
  async function runMediaCleanup(dryRun: boolean) {
    if (!requirePermission(canDeleteMedia)) return;
    setMediaCleanupBusy(true);
    try {
      const result = await apiFetch<ApiEnvelope<CleanupResult>>(
        "/api/v1/admin/media/cleanup",
        { method: "POST", body: JSON.stringify({ dry_run: dryRun }) },
        token,
      );
      if (dryRun) Message.info(`发现 ${result.data.candidate_count} 个孤立媒体候选`);
      else {
        Message.success(`已删除 ${result.data.deleted_count} 个孤立媒体`);
        await refresh();
      }
    } catch (err) {
      setError(String(err));
    } finally {
      setMediaCleanupBusy(false);
    }
  }

  const mediaURL = (asset: MediaAsset) =>
    asset.public_url.startsWith("http") ? asset.public_url : `${apiBase}${asset.public_url}`;
  async function copyMediaURL(asset: MediaAsset) {
    await navigator.clipboard.writeText(mediaURL(asset));
    Message.success("媒体地址已复制");
  }
  function insertMediaMarkdown(asset: MediaAsset) {
    if (!requirePermission(canInsertMediaMarkdown)) return;
    const current = postForm.getFieldValue("content_md") || "";
    const markdown = `![${asset.original_name}](${mediaURL(asset)})`;
    postForm.setFieldValue("content_md", current ? `${current}\n\n${markdown}` : markdown);
    Message.success("已插入 Markdown");
  }

  async function moderateComment(id: string, status: string) {
    if (!requirePermission(canModerateComments)) return;
    setCommentBusy(true);
    try {
      const result = await apiFetch<ApiEnvelope<Comment>>(
        `/api/v1/admin/comments/${id}/moderation`,
        { method: "PATCH", body: JSON.stringify({ status }) },
        token,
      );
      setComments((current) => current.map((item) => (item.id === id ? result.data : item)));
  ...
  async function deleteComment(id: string) {
    ...
    setCommentBusy(true);
    try {
      await apiFetch(`/api/v1/admin/comments/${id}`, { method: "DELETE" }, token);
  ...
  return {
    ...
    mediaCleanupBusy,
    commentBusy,
    createCategory: (values: CategoryFormValues) => createTaxonomy("categories", values),
    deleteCategory: (id: string) => deleteTaxonomy("categories", id),
    createTag: (values: TagFormValues) => createTaxonomy("tags", values),
    deleteTag: (id: string) => deleteTaxonomy("tags", id),
    saveSeries,
    deleteSeries,
    uploadMediaFile,
    deleteMedia,
