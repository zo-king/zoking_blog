...
export function useIdentityCommands({
  ...
  roleEditForm: FormInstance<{ name: string; description: string }>;
  passwordForm: FormInstance<{ password: string }>;
}) {
  ...
  async function createUser(values: UserValues) {
    ...
    setUserBusy(true);
    try {
      await apiFetch<ApiEnvelope<AdminUser>>(
        "/api/v1/admin/users",
  ...
  async function updateUserStatus(id: string, status: string) {
    ...
    setUserBusy(true);
    try {
      await apiFetch(`/api/v1/admin/users/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }, token);
  ...
  async function updateUserRoles(id: string, roleCodes: string[]) {
    ...
    try {
      await apiFetch(
        ...
        { method: "PATCH", body: JSON.stringify({ role_codes: roleCodes }) },
        token,
      );
  ...
  async function createRole(values: RoleValues) {
    ...
    setRoleBusy(true);
    try {
      await apiFetch("/api/v1/admin/roles", { method: "POST", body: JSON.stringify(values) }, token);
  ...
  async function updateRolePermissions(id: string, codes: string[]) {
    ...
    try {
      await apiFetch(
        ...
        { method: "PATCH", body: JSON.stringify({ permission_codes: codes }) },
        token,
      );
  ...
  function openRoleEditor(role: SystemRole) {
    setEditingRole(role);
    roleEditForm.setFieldsValue({ name: role.name, description: role.description });
  }
  ...
  async function updateRole(values: { name: string; description: string }) {
    if (!token || !editingRole || editingRole.is_system || !canManageRoles) return;
    setRoleBusy(true);
    try {
      await apiFetch(`/api/v1/admin/roles/${editingRole.id}`, { method: "PATCH", body: JSON.stringify(values) }, token);
  ...
  async function deleteRole(id: string) {
    ...
    setRoleBusy(true);
    try {
      await apiFetch(`/api/v1/admin/roles/${id}`, { method: "DELETE" }, token);
  ...
  async function resetUserPassword(values: { password: string }) {
    if (!token || !passwordResetUser || !canManageUsers) return;
    setUserBusy(true);
    try {
      await apiFetch(
        `/api/v1/admin/users/${passwordResetUser.id}/reset-password`,
        { method: "POST", body: JSON.stringify(values) },
        token,
      );
      Message.success("密码已重置，原有登录会话已撤销");
      closePasswordReset();
    } catch (err) {
      setError(String(err));
    } finally {
