...
export function AchievementsPage(props: AchievementsPageProps) {
  ...
  useEffect(() => {
    ...
    if (props.editorID === "new") {
      ...
      props.form.setFieldsValue({
        ...props.achievement,
        ended_at: props.achievement.ended_at || undefined,
        image_media_id: props.achievement.image_media_id || undefined,
      });
    }
  }, [props.achievement, props.editorID, props.form, props.mode]);

  if (props.mode === "list") {
    const years = Array.from({ length: Math.max(3, new Date().getFullYear() - 2023) }, (_, index) =>
        ...
        render: (_, record) => (
          <div className="achievement-title-cell">
            <Text ellipsis={{ showTooltip: true }}>
              <strong>{record.title || "未命名成果"}</strong>
            </Text>
            <Text type="secondary" ellipsis={{ showTooltip: true }}>
              {record.organization || "未填写组织"}
            </Text>
          </div>
        ),
      },
                ...
                >
                  <Tooltip content="删除成果">
                    <Button
                      ...
                      icon={<IconDelete />}
                      aria-label="删除成果"
                      loading={props.deletingID === record.id}
                    />
                  </Tooltip>
                </Popconfirm>
              ) : null}
              {canPublish && props.onStatusChange ? (
                <Tooltip content={record.status === "published" ? "归档成果" : "标记为已发布"}>
                  <Button
                    type="text"
                    size="mini"
                    icon={<IconSend />}
                    aria-label={record.status === "published" ? "归档成果" : "标记为已发布"}
                    onClick={() =>
                      props.onStatusChange?.(record.id, record.status === "published" ? "archived" : "published")
                    }
                  />
                </Tooltip>
              ) : null}
    ...
    return (
      <div className="achievements-page achievements-list-page">
        <PageHeader
          eyebrow="内容管理"
          title="成果"
          description="维护奖项、项目与认证等可公开展示的成果记录。"
          actions={
            canCreate || canPublish ? (
              <Space size={8} wrap>
                {canPublish ? (
                  <Button icon={<IconSend />} loading={props.publishing} onClick={() => void props.onPublish()}>
                    发布成果
                  </Button>
                ) : null}
                {canCreate ? (
                  <Button type="primary" icon={<IconPlus />} onClick={props.onNew}>
                    新建成果
                  </Button>
                ) : null}
              </Space>
        ...
        <ContentPanel className="achievements-list-panel">
          <div className="achievements-toolbar">
            <Select
              value={props.query.year || "all"}
              options={[
                { label: "全部年份", value: "all" },
                ...years.map((year) => ({ label: `${year} 年`, value: year })),
              ]}
              onChange={(value) => props.onQueryChange({ year: value === "all" ? "" : value })}
            />
          ...
          <Table<Achievement>
            ...
            pagination={{
              ...
              size: "small",
              showTotal: true,
              onChange: (page, pageSize) => props.onQueryChange({ page, pageSize }),
            }}
            noDataElement={
              <Empty
                description={
            ...
            onRow={(record) =>
              canUpdate ? { onClick: () => props.onSelect(record), style: { cursor: "pointer" } } : {}
            }
            columns={columns}
          />
        </ContentPanel>
  ...
  return (
    <div className="achievements-page achievements-editor-page">
      <PageHeader
        eyebrow="内容管理"
        title={props.editorID === "new" ? "新建成果" : "编辑成果"}
        description="完善成果信息、配图与公开状态。"
        actions={
          <Space wrap size={4}>
            <Button icon={<IconLeft />} onClick={props.onBack}>
              返回列表
            </Button>
            <Button
              icon={<IconSave />}
              disabled={!props.enabled || !(props.editorID === "new" ? canCreate : canUpdate)}
              loading={props.busy}
          ...
          <Row gutter={[24, 18]} align="stretch">
            <Col xs={24} xl={16} style={{ minWidth: 0 }}>
              <div className="achievement-editor-main">
                <Form.Item field="title" label="成果名称" rules={[{ required: true, message: "请输入成果名称" }]}>
                  <Input size="large" placeholder="例如：年度优秀开源项目" />
                </Form.Item>
                <Row gutter={[12, 0]}>
                  <Col xs={24} md={12}>
                    <Form.Item
                      field="organization"
                      label="组织 / 颁发方"
                      rules={[{ required: true, message: "请输入组织或颁发方" }]}
                    >
                      <Input placeholder="组织名称" />
                    </Form.Item>
                  </Col>
                  <Col xs={24} md={12}>
                    <Form.Item field="kind" label="成果类型" rules={[{ required: true, message: "请选择成果类型" }]}>
                      <Select
                        options={[
                ...
                <Form.Item field="summary" label="简介">
                  <Input.TextArea autoSize={{ minRows: 5, maxRows: 10 }} placeholder="用一两句话说明成果背景与价值。" />
                </Form.Item>
                <Row gutter={[12, 0]}>
                  <Col xs={24} md={12}>
                    <Form.Item
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item field="ended_at" label="结束日期">
                      <Input placeholder="可选，YYYY-MM-DD" />
                    </Form.Item>
                  </Col>
                </Row>
                <Form.Item field="external_url" label="外部链接">
            ...
            <Col xs={24} xl={8} style={{ minWidth: 0 }}>
              <aside className="achievement-editor-sidebar">
                ...
                <Form.Item field="status" label="状态">
                  <Select
                    disabled={!canPublish || props.editorID === "new"}
                    onChange={(status) => props.editorID && props.onStatusChange?.(props.editorID, status)}
                    options={[
                ...
                <Form.Item field="credential_id" label="凭证编号">
                  <Input placeholder="可选" />
                </Form.Item>
                <Form.Item field="sort_order" label="排序权重">
                  <InputNumber min={0} precision={0} mode="button" style={{ width: "100%" }} />
                </Form.Item>
                <div className="achievement-section-title">成果配图</div>
                <AchievementMediaPicker
                  value={imageMediaID}
                  selected={selectedMedia}
                  canRead={props.canReadMedia}
                  mediaURL={props.mediaURL}
                  search={props.searchMedia}
                  onChange={(value) => props.form.setFieldValue("image_media_id", value)}
                />
                {props.canDelete &&
                props.editorID &&
                props.editorID !== "new" &&
                props.achievement?.status !== "published" ? (
                  <Popconfirm
                    title="确认删除此成果？"
                    okText="删除"
                    cancelText="取消"
                    okButtonProps={{ status: "danger" }}
                    onOk={() => props.onDelete(props.editorID || "")}
                  >
                    <Button long status="danger" icon={<IconDelete />} loading={Boolean(props.deletingID)}>
                      删除成果
                    </Button>
                  </Popconfirm>
                ) : null}
