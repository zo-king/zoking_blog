import { Button, Form, Grid, Input, InputNumber, Space, Switch, type FormInstance } from "@arco-design/web-react";
    ...
    <>
      <PageHeader
        ...
        actions={
          props.canUpdate ? (
            <Space wrap size={8}>
              <Button icon={<IconSave />} loading={props.busy} onClick={() => props.form.submit()}>
                保存
              </Button>
              <Button icon={<IconEye />} loading={props.previewBusy} onClick={props.onPreview}>
                预览
              </Button>
              <Button type="primary" icon={<IconSend />} loading={props.busy} onClick={props.onPublish}>
                发布站点
              </Button>
            </Space>
          ) : undefined
          ...
          <Row gutter={[28, 18]} align="stretch">
              ...
              <section className="settings-section">
                ...
                <Row gutter={[14, 0]}>
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item
                      field="sidebar.emoji"
                      label="侧栏图标"
                      rules={[{ required: true, message: "请输入侧栏图标" }]}
                    >
                      <Input placeholder="✏️" />
                    </Form.Item>
                  </Col>
                ...
                <Form.Item
                  ...
                  rules={[
                    ...
                    { type: "url", message: "请输入有效的站点地址" },
                  ]}
                >
                  <Input placeholder="http://localhost:1313/" />
                </Form.Item>
                <Form.Item
                  field="sidebar.subtitle"
                  label="侧栏副标题"
                  rules={[{ required: true, message: "请输入侧栏副标题" }]}
                >
            ...
            <Col xs={24} xl={12} style={{ minWidth: 0 }}>
              <section className="settings-section settings-section-secondary">
                ...
                <Row gutter={[14, 0]}>
                  <Col xs={24} md={8}>
                    <Form.Item field="comments.enabled" label="启用评论" triggerPropName="checked">
                      <Switch />
                    </Form.Item>
                  </Col>
                  <Col xs={24} md={16}>
                    <Form.Item
                      ...
                      label="评论 API 地址"
                      rules={[
                        { required: true, message: "请输入评论 API 地址" },
                        { type: "url", message: "请输入有效的评论 API 地址" },
                      ]}
                    >
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item
                      field="footer.since"
                      label="页脚起始年份"
                      rules={[{ required: true, message: "请输入页脚起始年份" }]}
                    >
                ...
                {props.settingsHash ? (
                  <div className="settings-version">当前配置版本 {props.settingsHash.slice(0, 12)}</div>
                ) : null}
              </section>
            </Col>
          </Row>
