...
function TaxonomyIdentity({ item }: { item: Taxonomy }) {
  return (
    <div style={{ minWidth: 0 }}>
      ...
      <Text type="secondary" ellipsis={{ showTooltip: true }} style={{ display: "block", marginTop: 2, fontSize: 12 }}>
...
export function TaxonomyPage(props: Props) {
  ...
  function openSeriesModal(series?: Series) {
    ...
    props.seriesForm.setFieldsValue(
      series
        ? {
            name: series.name,
            slug: series.slug,
    ...
    .map((asset) => ({
      ...
      label: (
        <Space size={8}>
          ...
          <Text ellipsis>{asset.original_name || asset.filename}</Text>
        </Space>
      ),
    }));
        ...
        <Space size={8} style={{ minWidth: 0 }}>
          {record.cover_media ? (
            <Avatar shape="square" size={28}>
              <img src={props.mediaURL(record.cover_media)} alt="" />
            </Avatar>
          ) : null}
          <div style={{ minWidth: 0 }}>
            <Text bold ellipsis={{ showTooltip: true }} style={{ display: "block" }}>
        ...
        <Space size={2}>
          <Tooltip content="编辑系列">
            <Button
              ...
              size="mini"
              icon={<IconEdit />}
              aria-label={`编辑系列 ${record.name}`}
              disabled={props.busy}
              onClick={() => openSeriesModal(record)}
            />
          </Tooltip>
          <Popconfirm
            title="确认删除此系列？"
  ...
  return (
    <>
      <PageHeader title="内容组织" description="维护文章分类、标签与系列结构。" eyebrow="内容结构" />

      <ContentPanel
        className="tabbed-workbench taxonomy-workbench"
        title="内容组织管理"
        description="统一维护分类、标签和系列。"
      >
          ...
          <Tabs.TabPane key="categories" title="分类">
            <Space direction="vertical" size={16} style={{ width: "100%" }}>
              ...
              <Table<Taxonomy>
                ...
                columns={
                  props.canManage ? categoryColumns : categoryColumns.filter((column) => column.title !== "操作")
                }
                noDataElement="暂无分类"
                scroll={{ x: 620, y: 252 }}
              />
          ...
          <Tabs.TabPane key="series" title="系列">
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                ...
                {props.canManage ? (
                  <Button type="primary" icon={<IconPlus />} disabled={props.busy} onClick={() => openSeriesModal()}>
                    创建系列
                  </Button>
                ) : null}
              </div>
      ...
      {props.canManage ? (
        ...
        >
          ...
          >
            <Row gutter={12}>
              <Col xs={24} md={12}>
                <Form.Item field="name" label="名称" rules={[{ required: true, message: "请输入系列名称" }]}>
                  <Input placeholder="例如：Go 工程实践" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item field="slug" label="Slug" rules={[{ required: true, message: "请输入系列 Slug" }]}>
                  <Input placeholder="例如：go-engineering" />
                </Form.Item>
              </Col>
              <Col span={24}>
                <Form.Item field="description" label="说明">
                  <Input.TextArea autoSize={{ minRows: 2, maxRows: 3 }} placeholder="说明该系列覆盖的文章范围" />
                </Form.Item>
              </Col>
              <Col span={24}>
                <Form.Item field="cover_media_id" label="封面媒体">
                  <Select
                    allowClear
                    placeholder="从媒体库选择图片"
                    options={coverOptions}
                    notFoundContent="媒体库中暂无图片"
                  />
                </Form.Item>
              </Col>
              <Col xs={12} md={8}>
                <Form.Item field="sort_order" label="展示顺序">
