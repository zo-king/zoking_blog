...
export function UserManagementPage(props: Props) {
        ...
        props.canManageUsers ? (
          <Space size="mini">
            ...
            <Button
              ...
              icon={<IconLock />}
              disabled={props.userBusy}
              onClick={() => props.onResetPassword(record)}
            >
              重置密码
            </Button>
          </Space>
  ...
  const roleColumns: TableColumnProps<SystemRole>[] = [
    ...
    },
    {
      ...
      fixed: "right",
      render: (_, record) =>
        !record.is_system && props.canManageRoles ? (
          <Space size="mini">
            <Button size="mini" icon={<IconEdit />} disabled={props.roleBusy} onClick={() => props.onEditRole(record)}>
              编辑
            </Button>
            <Popconfirm
              title="确认删除此角色？"
            ...
            >
              <Button size="mini" status="danger" icon={<IconDelete />} loading={props.roleBusy}>
                删除
              </Button>
            </Popconfirm>
          </Space>
  ...
  return (
    <>
      <PageHeader title="用户与权限" description="管理后台账号、角色与访问边界。" />
      ...
      <Tabs defaultActiveTab="accounts">
        <TabPane key="accounts" title={`账号 (${props.userPagination.total})`}>
          <ContentPanel
            ...
            actions={
              props.canManageUsers ? (
                <Button type="primary" size="small" icon={<IconUserAdd />} onClick={openUserModal}>
                  创建账号
                </Button>
              ) : undefined
            }
            ...
            <ContentPanel
              ...
              actions={
                props.canManageRoles ? (
                  <Button type="primary" size="small" icon={<IconPlus />} onClick={openRoleModal}>
                    创建角色
                  </Button>
                ) : undefined
              }
          ...
          <Row gutter={16}>
            ...
            <Col xs={24} md={12}>
              <Form.Item
                field="password"
                label="初始密码"
                rules={[{ required: true, minLength: 10, maxLength: 72, message: "密码长度需为 10 至 72 个字符" }]}
              >
                ...
                rules={[
                  {
                    ...
                    message: "请输入 3 至 64 位小写字母、数字或下划线",
                  },
                ]}
              >
