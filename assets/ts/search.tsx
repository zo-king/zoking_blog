...
class Search {
    ...
    private async searchKeywords(keywords: string[]) {
        ...
        for (const item of rawData) {
            ...
            result.matchCount = titleMatches.length * 5 + contentMatches.length;
    ...
    private async doSearch(keywords: string[]) {
        ...
        try {
            ...
            this.showError(keywords);
        } finally {
            this.form.removeAttribute('aria-busy');
        }
    }

    private showError(keywords: string[]) {
        this.clear();
        ...
        message.textContent = '搜索索引加载失败，请检查网络后重试。';

        const retry = document.createElement('button');
        retry.className = 'search-result__retry';
        retry.type = 'button';
        retry.textContent = '重新搜索';
        retry.addEventListener('click', () => void this.doSearch(keywords));
                ...
                <a href={item.permalink}>
                    ...
                    {item.image &&
                        <div class="article-image">
                            <img src={item.image} alt={item.imageAlt || '文章封面'} loading="lazy" />
                        </div>
                    }
                </a>
            </article>
