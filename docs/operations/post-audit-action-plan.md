...
服务级进展（2026-08-10）：仓库已新增一键隔离演练脚本，强制使用内部 Docker 网络、专用临时 volumes 且不映射主机端口，覆盖 `/readyz`、公开读取、管理员登录及隔离发布，并在退出时删除 identity 和全部临时资源。脚本已通过本地及物理机 Bash 语法检查；真实演练待从密码管理器临时导出 identity 后执行。
...
- 管理员密码遗失恢复入口已固化为 `scripts/ops/reset-admin-password.sh`；执行时自动生成前后两份加密备份、撤销旧会话并写审计记录。首次生产执行和后续季度轮换仍需记录结果。
