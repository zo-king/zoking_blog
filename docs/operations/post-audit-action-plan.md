...
恢复验证：隔离环境已使用托管前的临时 identity 完整解密 PostgreSQL dump、媒体和发布/统计归档；identity 随后从物理机删除。后续服务级结果见 P0-3。

服务级验收（2026-08-10）：备份 `20260810T063709Z` 已在内部 Docker 网络和专用临时 volumes 中完成 API、Worker 与 Site 恢复，`/readyz`、公开读取、管理员登录和隔离发布全部通过；RTO 183 秒、RPO 501 秒。identity、解密数据、临时容器、网络和 volumes 均已删除，生产容器未重建。记录见 [restore-drill-2026-08-10.md](restore-drill-2026-08-10.md)。
...
### 3. 完成一次隔离恢复演练（服务级已完成：2026-08-10）
...
数据级验收结果：数据库和四类文件归档均可恢复；总演练约 28 秒；所有临时容器和 volumes 已清理。

服务级验收：使用备份 `20260810T063709Z` 启动隔离 PostgreSQL、API、Worker 和 Site，管理员登录与一次隔离发布均通过；观测 RTO 183 秒、RPO 501 秒，临时资源和 identity 全部清理。完整证据见 [restore-drill-2026-08-10.md](restore-drill-2026-08-10.md)。
...
进展：已确定使用飞书群机器人（2026-08-08）。仓库脚本现按飞书文本机器人协议发送并校验业务成功码；待在服务器 `/etc/zoking-blog/ops.env` 写入 URL 并完成测试。
...
- 将健康检查、备份、证书和 WireGuard 指标接入长期监控并保留至少 30 天执行结果（已完成：2026-08-10）。物理机与 Azure 均已安装 `infra/systemd/journald.conf.d/30-zoking-blog.conf`，显式启用持久化 journal 和 `MaxRetentionSec=30d`；重启 journald 后配置生效，当前占用分别为 32.0M 与 131.9M。四个 backup/health/edge/remote-maintenance timers 均为 `enabled/active`，30 天查询窗口可读取最新执行记录，Edge 最新检查全部通过。
