...
在 `/post/` 使用 Hugo `RegularPages.GroupByDate` 生成文章年表：

- `/archives/` 继续专用于后台可管理的获奖成果时间线，不混入文章。
- 年份导航、每年篇数、发布日期、标题、首个分类和阅读时长全部静态输出。
- 桌面使用三列紧凑行，移动端降为两列；标题触控目标至少 44px。
- 年份锚点只在允许动态效果时显示 900ms 定位高亮，`prefers-reduced-motion` 下关闭。
- 不分页、不采集浏览量、不称“热门”、不发起第三方请求。
- 无 JavaScript 时保留完整年份导航和文章列表。
